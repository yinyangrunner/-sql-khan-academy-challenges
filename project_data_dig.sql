...
SELECT Studio, ROUND(AVG(Domestic)) as avg_domestic, ROUND(MAX(domestic)) as max_domestic, ROUND(MIN(domestic)) as min_domestic FROM topmovies GROUP BY Studio;

SELECT Title, Domestic,
    CASE
        WHEN Domestic > 200 THEN "Uber_Blockbuster"
        WHEN Domestic > 200 THEN "Super_Blockbuster"
        WHEN Domestic > 100 THEN "Blockbuster"        
        ELSE "Not a Blockbuster"
    END as "Box_Office_Categorisation"
FROM topmovies
GROUP BY Box_Office_Categorisation 
;
